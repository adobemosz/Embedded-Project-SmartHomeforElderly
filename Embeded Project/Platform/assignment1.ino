...
// Pin ตัวส่ง (เหมือนเดิม)
#define DHTPIN 19
#define DHTTYPE DHT11
#define PIR_PIN 21
#define IR_PIN 32    // อย่าลืมต่อ IR ที่ขา 32 นะครับ
#define LDR_PIN 35
#define MQ_PIN 34

// ⚠️ ใส่ MAC Address ของ Gateway (ตัวปัจจุบัน) 
...
typedef struct struct_message {
  ...
  int humidity;
  int pir;
  int ir;       // ต้องมีบรรทัดนี้ครับ ถึงจะส่งค่าล้มไปได้
  int light;
  int smoke;
} struct_message;
...
void setup() {
  ...
  pinMode(IR_PIN, INPUT); // กำหนดขา IR
...
void loop() {
  ...
  // 3. แสดงผลครบทุกตัว (แก้ไขตรงนี้ให้แล้วครับ)
