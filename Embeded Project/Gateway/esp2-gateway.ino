...
// ตั้งค่าความดังเสียง (ถ้าดังเกินนี้ ให้ถือว่าขอความช่วยเหลือ)
...
// 🔥 ตัวแปรสถานะพิเศษ (ที่เพิ่มมาใหม่)
bool pir_latch_state = false; // ตัวจำค่า PIR (ติดค้าง)
...
// --- รับค่าปุ่มกดจาก Blynk ---

// V6: สวิตช์ Manual ไฟกลางคืน
BLYNK_WRITE(V6) {
  manual_night_light = param.asInt(); 
}

// V7: ปุ่ม Reset ไฟกันขโมย (PIR)
BLYNK_WRITE(V7) {
  if (param.asInt() == 1) {
...
void sendLineMessage(String message) {
  if (millis() - lastLineTime > 10000) { // กันรัว
    if (WiFi.status() == WL_CONNECTED) {
      WiFiClientSecure client;
      client.setInsecure(); 
  ...
  if (millis() - lastAiTime > 5000) {
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
        ...
        if ((label == "Help" || label == "help" || label == "_") && result.classification[ix].value > 0.7) {
            // ai_detected = true; // เปิดบรรทัดนี้ถ้าจะใช้ AI จริง
        }
    }
...
void processSystem() {
  ...
  // 1. ควัน -> ปั๊ม (แก้ให้เป็น Active LOW ตามที่เคยคุยกัน)
  ...
  // ============================================
  // 3. PIR (Logic ใหม่: D32 + Latch Mode) 🏃‍♂️
  // ============================================
  ...
  // สั่งเปิด/ปิดไฟ D32 ตามค่าที่ล็อคไว้
  ...
  // ============================================
  // 4. LDR (Logic ใหม่: Auto + Manual V6) 🌙
  // ============================================
  ...
  // 5. LED Alert (D18) สำหรับ IR ล้ม หรือสถานะอื่นๆ
  if (incomingReadings.ir == 0) {
